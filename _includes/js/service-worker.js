...
// ⚡️ DANGER ZONE ⚡️
// ================
// {% if jekyll.environment == 'production' or site.hdi.offline.development %}

// The shell cache keeps "landmark" resources, like CSS and JS, web fonts, etc.
// which won't change between content updates.
// {% capture cache_version %}v{{ site.hdi.offline.cache_version | default:"1" }}{% endcapture %}
// {% capture site_key %}sw{{ '/' | relative_url }}{% endcapture %}
const SHELL_CACHE = "shell-9.2.1--{{ cache_version }}--{{ site_key }}";

// A separate assets cache that won't be invalidated when there's a newer version of hdi.
...
const SHELL_FILES = [
  "{{ '/assets/css/hdi-9.2.1.css' | relative_url }}",
...
const PRE_CACHED_ASSETS = [
  ...
  /*{% for file in site.hdi.offline.precache_assets %}*/"{% include_cached smart-url url=file %}",
  /*{% endfor %}*/
];
...
async function onActivate() {
  ...
  return Promise.all(
    keys
      // Only consider caches created by this baseurl, i.e. allow multiple hdi installations on same domain.
